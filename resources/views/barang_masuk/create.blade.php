...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-400">Masukkan data barang yang baru datang ke gudang.</p>
        </div>
        <a href="{{ route('barang-masuk.index') }}" class="text-sm text-slate-500 hover:text-slate-700">Kembali ke daftar</a>
    </div>
    ...
    <form action="{{ route('barang-masuk.store') }}" method="POST" class="bg-white p-6 rounded-xl shadow-sm border">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="tanggal_masuk" class="mt-1 block w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
            </div>
        ...
        <div class="mt-6 flex items-center justify-end space-x-3">
            <a href="{{ route('barang-masuk.index') }}" class="inline-block px-4 py-2 bg-slate-50 text-slate-600 border border-slate-200 rounded-md hover:bg-slate-100">Batal</a>
            <button type="button" onclick="confirmCreate(this.closest('form'), 'Barang Masuk')" class="inline-flex items-center px-4 py-2 bg-emerald-600 text-white rounded-md shadow hover:bg-emerald-700 transition">Simpan</button>
        </div>
    </form>
</div>
