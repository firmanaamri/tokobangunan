...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">Edit Barang Keluar</h1>
            <p class="text-sm text-slate-400">Perbarui data pengeluaran barang.</p>
        </div>
    ...
    <form action="{{ route('barang-keluar.update', $item) }}" method="POST" class="bg-white p-6 rounded-xl shadow-sm border">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="col-span-1">
                ...
                <select id="kategori_filter" class="mt-1 block w-full border border-slate-200 rounded-md px-3 py-2 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
                    ...
                    @foreach($kategoris as $k)
                        <option value="{{ $k->id }}" {{ $item->barang->kategori_id == $k->id ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-span-1">
                ...
                <select id="barang_select" name="barang_id" class="mt-1 block w-full border border-slate-200 rounded-md px-3 py-2 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
                    ...
                    @foreach($barangs as $b)
                        <option value="{{ $b->id }}" data-kategori="{{ $b->kategori?->id }}" {{ $item->barang_id == $b->id ? 'selected' : '' }}>{{ $b->nama_barang }} <span class="text-xs">(SKU: {{ $b->sku }})</span></option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="jumlah_barang_keluar" value="{{ $item->jumlah_barang_keluar }}" class="mt-1 block w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300" min="1" required>
            </div>

            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1">Tanggal</label>
                <input type="date" name="tanggal_keluar" value="{{ $item->tanggal_keluar->format('Y-m-d') }}" class="mt-1 block w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">
            </div>

            <div class="md:col-span-2">
                <label class="block text-xs font-semibold text-slate-600 mb-1">Keterangan</label>
                <textarea name="keterangan" rows="3" class="mt-1 block w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300">{{ $item->keterangan }}</textarea>
            </div>
        </div>
