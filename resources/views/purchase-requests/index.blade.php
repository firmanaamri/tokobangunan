    ...
    <div class="max-w-7xl mx-auto">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-6">
            <div class="w-full">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Contoh: PR000001" class="w-full p-2.5 rounded-lg border border-gray-300 bg-white focus:border-blue-500 outline-none transition-all" />
            </div>
            <div class="w-full">
                <label class="block text-sm text-slate-600 mb-1">Status:</label>
                <select name="status" class="w-full p-2.5 rounded-lg border border-gray-300 bg-white focus:border-blue-500 outline-none transition-all cursor-pointer">
            ...
            <div class="flex gap-2 w-full">
                ...
                <a href="{{ route('purchase-requests.index') }}" class="px-4 py-2.5 bg-red-100 hover:bg-red-200 text-red-700 border border-red-200 rounded-lg transition shadow-sm font-medium text-center">Reset</a>
            </div>
        </form>
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-gradient-to-r from-slate-800 to-slate-700 text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-sm font-bold">Supplier (Pengajuan)</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors duration-200">
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-slate-500 mt-1">{{ $pr->created_at->format('d M Y') }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    ...
                                    $currentClass = $statusClasses[$pr->status] ?? 'bg-slate-100';
                                @endphp
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex gap-2 justify-center">
                                    {{-- Tombol Lihat Detail --}}
                                    <a href="{{ route('purchase-requests.show', $pr) }}" class="bg-blue-500 hover:bg-blue-600 text-white w-9 h-9 inline-flex items-center justify-center rounded-lg transition-colors shadow-sm" title="Lihat Detail">
                                    ...
                                    {{-- Tombol Edit & Hapus (Hanya jika Pending & Milik User) --}}
                                    @if($pr->status === 'pending' && $pr->user_id === auth()->id())
                                        <a href="{{ route('purchase-requests.edit', $pr) }}" class="bg-amber-500 hover:bg-amber-600 text-white w-9 h-9 inline-flex items-center justify-center rounded-lg transition-colors shadow-sm" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                            </svg>
                                        </a>

                                        <form action="{{ route('purchase-requests.destroy', $pr) }}" method="POST" onsubmit="return confirm('Yakin ingin membatalkan pengajuan ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white w-9 h-9 inline-flex items-center justify-center rounded-lg transition-colors shadow-sm" title="Hapus">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
